...
pipeline {
  ...
  */
  ...
  stages {
    ...
    stage('deploy') {
      ...
      steps {
		...
		def jobBuild = build(
          ...
          if(log.contains("Invalid JWT Signature "))
		  {
    ...
    stage('check the status of last deploy') {
        steps {
            script{
                def jobBuild = Jenkins.getInstance().getItemByFullName(env.DEPLOY_JOB_NAME)
		        def last_build = jobBuild.getLastBuild()
		        echo "$last_build"
		        def result = last_build.getResult()
		        echo "$result"
