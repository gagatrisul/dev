    ...
    stage('Pull repo') {
      steps {
        ...
        sh script: """echo ${VAULT_PASS} > pwd.txt
                      ansible-vault decrypt config/secret.properties.ca.dev3 config/secret.properties.ca.tst4 config/secret.properties.ca.tst6 config/secret.properties.ca.ppd3 config/secret.properties.prd --vault-password-file=pwd.txt
                      cat config/secret.properties.ca.dev3 >> config/local-specific.properties.ca.dev3
                      cat config/secret.properties.ca.tst4 >> config/local-specific.properties.ca.tst4
                      cat config/secret.properties.ca.tst6 >> config/local-specific.properties.ca.tst6
                      cat config/secret.properties.ca.ppd3 >> config/local-specific.properties.ca.ppd3
                      cat config/secret.properties.prd >> config/local-specific.properties.prd
                      rm -f pwd.txt"""
        }
      }
    }
      ...
      steps{
        script {
          ...
          if (env.DEPLOY_ENV != 'NONE') {
            build job: "ecom/ecom-ca-${DEPLOY_ENV}-deploy", parameters: [string(name: 'BUILD_VERSION', value:"${buildName}")], wait:false
            script {
